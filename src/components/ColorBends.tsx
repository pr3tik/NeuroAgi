// ColorBends.tsx — React Bits three.js flowing gradient background.
// Source: exact original from React Bits integration docs.
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Use performance.now() — THREE.Clock is deprecated in recent three.js
  ...
  }, [rotation, autoRotate, speed, scale, frequency, warpStrength, mouseInfluence,
      parallax, noise, iterations, intensity, bandWidth, colors, transparent]);
  ...
  useEffect(() => {
    ...
    // Window-level listeners: bypass pointer-events:none on the wrapper div
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const rect = container.getBoundingClientRect();
      pointerTargetRef.current.set(
        ((t.clientX - rect.left) / (rect.width  || 1)) * 2 - 1,
        -(((t.clientY - rect.top) / (rect.height || 1)) * 2 - 1)
      );
    };
