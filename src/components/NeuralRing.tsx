...
import { Send, Square, Plus, ThumbsUp, ThumbsDown, Check, RotateCcw, Play, Mic, Sparkles } from "lucide-react";
...
// Manus/Claude-style work receipt: ONE pill per answer. Live, it narrates the current
// step; done, it collapses to a source count. Expanded, it shows the full timeline in
// pull order — every step, every source it returned, nothing sliced away — plus the
// turn's final source list and the trace id for debugging. This replaces the old flat
// chips row (kept only as a fallback for legacy messages with no activity data).
function ActivityDropdown({ steps, live, sources, traceId }: {
                ...
                messages.map((m, i) => (
                  <div key={i} className="nr-msg-in" style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "84%" }}>
                    ...
                    >
                      ...
                      {m.role === "assistant" && m.activity && <ActivityDropdown steps={m.activity} live={false} sources={m.sources} traceId={m.traceId} />}
                      {m.role === "assistant"
                        ? <Markdown remarkPlugins={[remarkGfm]}>{m.content}</Markdown>
                        : m.content}
                      {m.quiz && <InlineQuiz cards={m.quiz} userId={userId} courseId={null} />}
                      {/* Legacy fallback ONLY: flat source chips for messages that predate
                          activity capture. Answers with m.activity show sources inside the
                          ActivityDropdown (pull order, complete) — not here. */}
                      {!m.activity && m.sources?.length > 0 && (
