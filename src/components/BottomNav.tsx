...
function Icon({ name }) {
  ...
  switch (name) {
    ...
    case "studyAssistant": return <svg {...common}><path d="M4 19V6a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v13" /><path d="M4 19h16" /><path d="M9 4v9l3-2 3 2V4" /></svg>;
    case "connections": return <svg {...common}><path d="M10 13a5 5 0 0 0 7 0l3-3a5 5 0 0 0-7-7l-1 1" /><path d="M14 11a5 5 0 0 0-7 0l-3 3a5 5 0 0 0 7 7l1-1" /></svg>;
    default:            return null;
  }
}
...
export default function BottomNav({ currentPage, onNavigate, collapsed = false, onToggleCollapse }) {
  ...
  if (isWide) {
    // `collapsed` is the PINNED state (drives the page's left margin in App.tsx). When
    // pinned collapsed, the rail still expands on hover — `vis` is the effective look.
    // The expanded rail overlays the page (position:fixed, high z-index) so hovering
    // never reflows content. Pin it open with the toggle to keep it expanded.
    const vis = collapsed && !hover;
      ...
      <aside
        ...
        <div style={{
          ...
          {!vis && (
            ...
            >
              {/* pin icon when hover-open (click to keep), chevron when pinned open (click to collapse) */}
              {collapsed ? (
