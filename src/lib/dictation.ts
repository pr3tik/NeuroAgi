...
// the input box (the user edits/sends — never auto-sent). Engine choice:
//   1. Web Speech API (Chrome/Edge/Safari): instant streaming interim results, no
//      server round-trip, no cost.
//   2. Fallback (Firefox etc.): MediaRecorder → POST /api/stt (Groq Whisper) — one
//      final transcript when the user taps stop.
// Pure logic module (no React) so both engines are unit-testable.
...
export interface Dictation {
  ...
  readonly engine: "webspeech" | "server";
